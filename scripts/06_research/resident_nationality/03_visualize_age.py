...
OUTPUT_DATA = OUTPUT_DIR / "01_定住者_主要国籍別_年代構成_2024.csv"
OUTPUT_CHART = VIS_DIR / "01_定住者_ブラジル中国フィリピン_年代構成_積み上げ割合_2024.png"
...
# ======================== データ作成 ========================
def make_age_composition(input_file: Path, age_map_file: Path) -> pd.DataFrame:
    """2024年の定住者について、主要国籍別の年代構成比を作成する。"""
    df = pd.read_csv(input_file, encoding="utf-8-sig")
    df = df[
        (df["年"] == TARGET_YEAR)
        & (df["在留資格"] == TARGET_STATUS)
        & (df["国籍・地域"].isin(TARGET_COUNTRIES))
    ].copy()
    ...
    # レポート本体の年代構成とそろえ、不詳は高齢層側へ寄せる。
    df["年代"] = df["年代"].replace({"不詳": "70代以上"})

    df = (
        df.groupby(["年", "国籍・地域", "年代"], as_index=False, dropna=False)["人口"]
        .sum()
    )
    df["総人口"] = df.groupby(["年", "国籍・地域"])["人口"].transform("sum")
    df["構成比"] = df["人口"] / df["総人口"]
    df["構成比_pct"] = df["構成比"] * 100

    all_rows = pd.MultiIndex.from_product(
        [[TARGET_YEAR], TARGET_COUNTRIES, AGE_ORDER],
        names=["年", "国籍・地域", "年代"],
    ...
    df["構成比"] = df["人口"] / df["総人口"]
    df["構成比_pct"] = df["構成比"] * 100
    df["国籍・地域"] = pd.Categorical(
        df["国籍・地域"],
        categories=TARGET_COUNTRIES,
        ordered=True,
    )
    df["年代"] = pd.Categorical(df["年代"], categories=AGE_ORDER, ordered=True)
    return df.sort_values(["国籍・地域", "年代"]).reset_index(drop=True)


# ======================== 可視化 ========================
def plot_age_composition(df: pd.DataFrame, output_file: Path) -> None:
    """主要国籍別の年代構成を100%積み上げ横棒グラフで出力する。"""
    pivot = (
        df.pivot(index="国籍・地域", columns="年代", values="構成比_pct")
    ...
    ax.set_title("定住者 主要国籍別の年代構成（2024年）", pad=14)
    ax.set_xlabel("構成比（%）")
    ax.set_ylabel("国籍・地域")
    ax.set_xlim(0, 100)
...
if __name__ == "__main__":
    ...
    composition = make_age_composition(INPUT_FILE, AGE_MAP_FILE)
    composition.to_csv(OUTPUT_DATA, index=False, encoding="utf-8-sig")
    plot_age_composition(composition, OUTPUT_CHART)

    print(f"可視化用データ: {OUTPUT_DATA}")
    print(f"グラフ: {OUTPUT_CHART}")
    print("処理完了")
